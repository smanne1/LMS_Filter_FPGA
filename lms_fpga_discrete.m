...
filter_size=8;
w0=0.5;  phi=0;
desired_signal=[0 1 0 1 0 1 0 1]*fps;
input_signal=[1 1 0 1 1 0 1 1]*fps;

%desired_signal=10*sin(2*pi*[0:0.1:N-1]*w0+phi);
%input_signal=10*si11n(2*pi*[0:0.1:N-1]*w0+phi).^2;
...
filter_output(i)=dot(filter_current, input_vector)/fps
koutput(i)=filter_output(i);
error= desired_signal(i)-filter_output(i)

step_size=round(fps*fps/(0.001*fps+(dot(input_vector, input_vector))/fps))
%step_size=2*0.0000001
filter_current = filter_current +round(step_size*error*input_vector/(fps*fps))
...
%output=conv((filter_current)',input_signal)/(fps*fps)
%koutput=output(1:10)
%for i=1:filter_size
%input_vector(1)=input_signal(i);
%koutput(i)=dot(filter_current, input_vector)/(fps*fps);
%for j=filter_size:-1:2
%input_vector(j)=input_vector(j-1);
%end
%end
